...
<select name="nama_wad" id="nama_wad">
				<?php
				$sa = "SELECT * FROM wad  ORDER BY wad ASC";
				$qa = $pdo->prepare($sa);
				$qa->execute();
...
<tr><td><label>Assgin: </label></td><td>
<select name="nama" id="nama">
				<?php
				$sa = "SELECT * FROM akaun  ORDER BY id_pengguna ASC";
				$qa = $pdo->prepare($sa);
				$qa->execute();
				
				while ($ta = $qa->fetch(PDO::FETCH_OBJ)) {
					echo '<option value="' . $ta->nama . '">' . $ta->nama . '</option>';
				}
				?>
			  </select></td></tr>
